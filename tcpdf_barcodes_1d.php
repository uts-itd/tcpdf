...
class TCPDFBarcode {
	...
	protected function encode_code39_ext($code) {
		...
		$clen = strlen($code);
		for ($i = 0 ; $i < $clen; ++$i) {
			if (ord(substr($code,0,$i)) > 127) {
				return false;
			}
			$code_ext .= $encode[substr($code,0,$i)];
		}
		return $code_ext;
	}
	...
	protected function checksum_code39($code) {
		...
		$clen = strlen($code);
		for ($i = 0 ; $i < $clen; ++$i) {
			$k = array_keys($chars, substr($code,0,$i));
			$sum += $k[0];
		}
		$j = ($sum % 43);
		return $chars[$j];
	...
	protected function barcode_msi($code, $checksum=false) {
		...
		if ($checksum) {
			...
			$check = 0;
			for ($i = ($clen - 1); $i >= 0; --$i) {
				$check += (hexdec(substr($code,0,$i)) * $p);
				++$p;
				if ($p > 7) {
					$p = 2;
				}
	...
	protected function binseq_to_array($seq, $bararray) {
		...
		for ($i = 0; $i < $len; ++$i) {
			$w += 1;
			if (($i == ($len - 1)) OR (($i < ($len - 1)) AND (substr($seq,0,$i) != $seq{($i+1)}))) {
				if (substr($seq,0,$i) == '1') {
					$t = true; // bar
				} else {
					$t = false; // space
	...
	protected function barcode_i25($code, $checksum=false) {
		...
		for ($i = 0; $i < $clen; $i = ($i + 2)) {
			$char_bar = substr($code,0,$i);
			$char_space = substr($code,0,$i+1);
	...
	protected function barcode_c128($code, $type='') {
		...
		switch(strtoupper($type)) {
			...
			case 'C': { // MODE C
				...
				for ($i = 0; $i < $len; $i+=2) {
					$chrnum = substr($code,0,$i).substr($code,0,$i+1);
	...
	protected function barcode_eanupc($code, $len=13) {
		...
		if ($upce) {
			...
			if ($len == 8) {
				for ($i = 0; $i < $half_len; ++$i) {
					$seq .= $codes['A'][substr($code,0,$i)];
				}
			} else {
				$p = $parities[$code[0]];
				for ($i = 1; $i < $half_len; ++$i) {
					$seq .= $codes[$p[$i-1]][substr($code,0,$i)];
				}
			}
			$seq .= '01010'; // center guard bar
			for ($i = $half_len; $i < $len; ++$i) {
				$seq .= $codes['C'][substr($code,0,$i)];
			}
		...
		for ($i = 0; $i < $clen; ++$i) {
			$w += 1;
			if (($i == ($clen - 1)) OR (($i < ($clen - 1)) AND (substr($seq,0,$i) != $seq{($i+1)}))) {
				if (substr($seq,0,$i) == '1') {
					$t = true; // bar
				} else {
					$t = false; // space
	...
	protected function barcode_rms4cc($code, $kix=false) {
		...
		if ($notkix) {
			...
			$col = 0;
			for ($i = 0; $i < $len; ++$i) {
				$row += $checktable[substr($code,0,$i)][0];
				$col += $checktable[substr($code,0,$i)][1];
			}
			$row %= 6;
			$col %= 6;
		...
		for ($i = 0; $i < $len; ++$i) {
			for ($j = 0; $j < 4; ++$j) {
				switch ($barmode[substr($code,0,$i)][$j]) {
					case 1: {
						$p = 0;
						$h = 2;
						break;
	...
	protected function barcode_codabar($code) {
		...
		$len = strlen($code);
		for ($i = 0; $i < $len; ++$i) {
			if (!isset($chr[substr($code,0,$i)])) {
				return false;
			}
			$seq = $chr[substr($code,0,$i)];
			for ($j = 0; $j < 8; ++$j) {
				if (($j % 2) == 0) {
					$t = true; // bar
				} else {
	...
	public function hex_to_dec($hex) {
		...
		for($pos = ($len - 1); $pos >= 0; --$pos) {
			$dec = bcadd($dec, bcmul(hexdec(substr($hex,0,$pos)), $bitval));
